...
export function CallOverlay({ call, contact, script, onMuteToggle, onEndCall }) {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-4 md:p-8 relative">
      ...
      <div className="relative w-full max-w-lg flex flex-col items-center z-10">
        ...
        <div className="relative mb-6 md:mb-12">
          ...
          <div className="relative group">
            <div className={`
              w-24 h-24 md:w-32 md:h-32 rounded-[30px] md:rounded-[40px] flex items-center justify-center text-3xl md:text-4xl font-black text-white shadow-2xl transition-all duration-700 transform group-hover:scale-105
              ${status === 'live' 
            ...
            {status === 'live' && (
              <div className="absolute -top-2 -right-2 md:-top-3 md:-right-3 w-8 h-8 md:w-10 md:h-10 rounded-xl md:rounded-2xl bg-white dark:bg-card border-4 border-background flex items-center justify-center shadow-lg animate-bounce">
                <Volume2 className="w-4 h-4 md:w-5 h-5 text-green-500" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="text-center space-y-1 md:space-y-2 mb-6 md:mb-8">
          ...
          <div className="flex items-center justify-center gap-2">
            <span className="px-3 py-1 rounded-full bg-muted/50 text-[10px] md:text-xs font-mono font-bold text-muted-foreground tracking-widest uppercase">
              {contact?.number}
            </span>
          </div>
        </div>
        ...
        <div className={`flex items-center gap-2.5 px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-2xl border ${cfg.bg} border-white/5 backdrop-blur-xl transition-all duration-500 mb-6 md:mb-10`}>
          <div className={`w-2 h-2 md:w-2.5 md:h-2.5 rounded-full ${cfg.dot}`} />
          <span className={`text-xs md:text-sm font-black uppercase tracking-widest ${cfg.color}`}>{cfg.label}</span>
        </div>

        {/* Waveform Visualization */}
        <div className="w-full flex items-center justify-center gap-1 md:gap-1.5 h-12 md:h-16 mb-6 md:mb-12">
          {Array.from({ length: 28 }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className={`w-[3px] md:w-[4px] rounded-full transition-all duration-150 ${
        ...
        {/* Error Message */}
        {error && (
          <div className="mb-6 md:mb-8 p-4 bg-destructive/10 border border-destructive/20 rounded-2xl text-destructive text-sm font-bold flex items-center gap-3 animate-shake">
          ...
          <div className="flex items-center gap-6 md:gap-8">
            ...
            <div className="flex flex-col items-center gap-2 md:gap-3">
              <button
                ...
                className={`
                  w-12 h-12 md:w-16 md:h-16 rounded-2xl md:rounded-3xl flex items-center justify-center transition-all duration-300 transform active:scale-90
                  ${isMuted
            ...
            <div className="flex flex-col items-center gap-2 md:gap-3">
              <button
                onClick={onEndCall}
                className="w-16 h-16 md:w-24 md:h-24 rounded-2xl md:rounded-[36px] bg-red-500 text-white flex items-center justify-center shadow-2xl shadow-red-500/40 hover:bg-red-600 hover:scale-110 active:scale-95 transition-all duration-300"
              >
                <PhoneOff className="w-6 h-6 md:w-10 md:h-10" />
              </button>
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-red-500">
                End Call
              </span>
            </div>
            ...
            <div className="flex flex-col items-center gap-2 md:gap-3">
              <button className="w-12 h-12 md:w-16 md:h-16 rounded-2xl md:rounded-3xl bg-card border-2 border-border text-muted-foreground flex items-center justify-center opacity-50 cursor-not-allowed">
                <Volume2 className="w-5 h-5 md:w-6 md:h-6" />
              </button>
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-muted-foreground/50">
                Speaker
              </span>
            </div>
          </div>
