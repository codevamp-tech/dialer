  ...
  return (
    <div className="h-full flex flex-col bg-background p-8 overflow-hidden relative">
      ...
      <div className="max-w-7xl w-full mx-auto flex flex-col h-full z-10">
        ...
        {/* Header Section */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 animate-slide-in-up">
          <div>
            <h1 className="text-4xl font-black tracking-tighter text-foreground">Call Archives</h1>
            <p className="text-muted-foreground font-medium mt-1">Review and manage your telecalling performance</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="p-3 bg-card border border-border rounded-2xl hover:bg-muted transition-colors shadow-premium">
              <Filter className="w-5 h-5 text-muted-foreground" />
            </button>
            <button className="p-3 bg-primary text-white rounded-2xl hover:opacity-90 transition-all shadow-xl shadow-primary/20">
        ...
        {/* Table Container */}
        <div className="flex-1 bg-card/60 backdrop-blur-xl border border-border rounded-[32px] overflow-hidden flex flex-col shadow-premium animate-fade-in delay-75">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border bg-muted/20">
                    ...
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Duration</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground text-center">Outcome</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground"></th>
                  </tr>
                </thead>
                    ...
                    >
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-muted to-border border-2 border-background flex items-center justify-center text-foreground font-black text-lg shadow-sm group-hover:scale-110 transition-transform">
                            {(log.customerName || log.to).charAt(0).toUpperCase()}
                          </div>
                          <span className="font-bold text-foreground group-hover:text-primary transition-colors">{log.customerName || 'Unknown Lead'}</span>
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold font-mono tracking-tighter text-foreground/80">{log.to}</span>
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">Outbound</span>
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-foreground/80">{format(new Date(log.createdAt), 'MMM d, yyyy')}</span>
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">{format(new Date(log.createdAt), 'h:mm a')}</span>
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-2 text-sm font-black text-foreground/70">
                      ...
                      <td className="px-8 py-5">
                        <div className="flex justify-center">
                          <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-sm ${getDispositionStyles(log.disposition || log.status)}`}>
          ...
          {!dialerCallLogsLoading && filteredLogs.length > 0 && (
            <div className="px-8 py-4 bg-muted/20 border-t border-border flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-muted-foreground">
